<template>
  <div :class="$style['container']" @mousedown="mousedown" @mouseup="mouseup">
    <draggable v-if="canRemove" v-show="isRemove" :list="myArray1" group="list"
      :class="[$style['drag-group'], $style['drag-group-remove']]">
      <div v-for="element in myArray1" :key="element.dragId">
        <!-- {{ element.dragId }} -->
      </div>
    </draggable>
    <div v-if="canRemove" v-show="isRemove" ref="remove" :class="['can-remove', $style['remove']]">
      {{ placeholder }}
    </div>
    <draggable ref="draggable" :list="myArray" group="list" :class="$style['drag-group']" :move="move" @end="end"
      @change="change">
